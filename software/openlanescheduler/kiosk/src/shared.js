...
export const C = {
  ...
  kioskBg:'#0a0a12', kioskBgMid:'#12121f',
  kioskCyan:'#00ffff', kioskGreen:'#00ff88', kioskYellow:'#ffc800',
...
// dark (bottom-right) shadows cast on the single base `surface` color. Pass the
// palette from useColors(). Append hex alpha (e.g. `${color}4d`) for accent glows.
export const neuRaised = (C) => `5px 5px 12px ${C.shadowDark}, -5px -5px 12px ${C.shadowLight}`;
export const neuInset = (C) => `inset 4px 4px 9px ${C.shadowDark}, inset -4px -4px 9px ${C.shadowLight}`;

export const EMBEDDED = window.self !== window.top;

// Parse KIOSK_LANES from environment/config
export const KIOSK_LANES = (() => {
  const envLanes = import.meta.env.VITE_KIOSK_LANES || window.__KIOSK_LANES__ || '[1,2]';
  try {
    return JSON.parse(envLanes);
  } catch {
