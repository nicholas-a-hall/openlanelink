...
Every node addresses a **side** of its gateway's lane pair (`LaneSide::A` /
`LaneSide::B`), never a lane number — a gateway's mesh *is* one lane pair, so
a lane number carries nothing the mesh can use. Every leaf sketch is
therefore identical on every pair in the house, with `GATEWAY_MAC` the only
per-pair constant. Side → real lane number is resolved once, on the Pi, at
the mesh boundary (`uart_bridge/lane_map.py`). See `PROTOCOL.md`'s "Lane
sides, not lane numbers".
...
`NodeMessage{msgType, seq, code, laneSide, timestampMs, data[64]}` — see
...
## Fouling (break-beam) node — `foul_node.ino`
Covers **one lane pair**, one break-beam sensor per side. Registers as `NODE_FOULING`.

| | |
|---|---|
| **Emits** | `MSG_REGISTER{code=NODE_FOULING}` — boot + every 10s, **ESP-NOW only**.<br>`MSG_LANE_EVENT{code=LANE_FOUL\|LANE_CLEAR, laneSide}` — on every debounced (30ms) sensor edge, per side, **dual-sent on ESP-NOW and RS485**. |
| **Accepts** | Nothing, on either transport — no ESP-NOW recv callback, and nothing it reads off RS485 is acted on. It *does* read the bus (`rs485.observeBus()`), but purely so its send-side collision avoidance can see other nodes' traffic; the bytes are discarded. Pure sensor emitter. |
...
Covers **one lane pair**, two break-beams per side (upstream/downstream), 4
sensors total. Registers as `NODE_SPEED`. Does no timing/pairing math itself
— see `HANDOFF.md`'s "nodes are dumb" principle.

| | |
|---|---|
| **Emits** | `MSG_REGISTER{code=NODE_SPEED}` — boot + every 10s, **ESP-NOW only**.<br>`MSG_BEAM_EVENT{code=BEAM_BROKEN\|BEAM_CLEAR, laneSide, data[0]=beamRole}` — on every debounced (30ms) sensor edge, per beam, **dual-sent on ESP-NOW and RS485**. `beamRole`: 0=upstream, 1=downstream (2 is the ball detection node's, never emitted here). |
| **Accepts** | Nothing, on either transport — no ESP-NOW recv callback, and nothing it reads off RS485 is acted on. It *does* read the bus (`rs485.observeBus()`), but purely so its send-side collision avoidance can see other nodes' traffic; the bytes are discarded. Pure sensor emitter. |
| **Heartbeat** | Only the 10s `MSG_REGISTER` re-announce, ESP-NOW only. |

## Ball detection node — `ball_detect_node.ino`
Covers **one lane pair**, one break-beam sensor per side, sited just **before
the pin deck**. Registers as `NODE_BALL_DETECT`. Its one output is "a ball has arrived
...
| | |
|---|---|
| **Emits** | `MSG_REGISTER{code=NODE_BALL_DETECT}` — boot + every 10s, **ESP-NOW only**.<br>`MSG_BEAM_EVENT{code=BEAM_BROKEN\|BEAM_CLEAR, laneSide, data[0]=ROLE_BALL_DETECT}` — on every debounced (30ms) sensor edge, per side, **dual-sent on ESP-NOW and RS485**. Same message type as the speed node's, distinguished **only** by `beamRole` = 2 (vs. the speed node's 0/1). |
| **Accepts** | Nothing, on either transport — no ESP-NOW recv callback, and nothing it reads off RS485 is acted on. It *does* read the bus (`rs485.observeBus()`), but purely so its send-side collision avoidance can see other nodes' traffic; the bytes are discarded. Pure sensor emitter. |
| **Heartbeat** | Only the 10s `MSG_REGISTER` re-announce, ESP-NOW only. |
...
- **`MSG_SCORE_EVENT`s are received-but-ignored by every node today, and
  ...
  same fixed `NodeMessage` size, that accidental filter is gone, so the
  pinsetter's receive handler now names `MSG_SCORE_EVENT` (along with the
  lane and beam events it overhears on the shared bus) in an explicit
  silent-ignore branch, leaving its `default` branch to mean "genuinely
  unrecognized." **Any future receiving node has to make the same decision
  deliberately** — the size check will not make it for you. The fouling,
  speed, and ball detection nodes act on nothing regardless (no recv
  callback, and the frames they read off RS485 for carrier sense are
  discarded). There is no real consumer of `MSG_SCORE_EVENT` yet — that's a
  future display/UI node's job.
