...
| **Accepts** | `MSG_COMMAND{code=CommandCode, laneNumber}` from the gateway, on **either transport**: `CMD_CYCLE` (fire cycle solenoid once), `CMD_POWER_ON`/`CMD_POWER_OFF` (latch machine power), `CMD_RERACK` (sequence to a fresh full rack — 1 or 2 cycles depending on tracked ball state), `CMD_RESPOT` (**stub**, logs + acks only, not implemented), `CMD_STATUS` (request a `STATUS_MACHINE_STATUS` report). Also `MSG_ACK{code=MSG_STATUS, seq}` from the gateway on either transport, acking this node's own outbound `MSG_STATUS` retries. A single dispatcher (`handleIncomingNodeMessage`) handles both, regardless of which wire the message came in on. |
...
The hub. Not itself discovered by anything (every other node hardcodes its MAC). Owns dynamic peer registration and cooldown logic; does no interval/timing math on sensor data (forwards it to the Pi as-is). Has RS485 hardware (pins UNVERIFIED placeholder, like every other node's) as the shared bus's other endpoint — every node's RS485 fallback traffic ultimately terminates here.

| | |
|---|---|
| **Emits (to pinsetter, dual-sent ESP-NOW + RS485)** | `MSG_COMMAND{CMD_RERACK, laneNumber}` on a fouling node's `LANE_FOUL` (subject to the foul cooldown below). Bench-console-triggered: `CMD_CYCLE`, `CMD_RERACK`, `CMD_RESPOT`, `CMD_POWER_ON/OFF`, `CMD_STATUS`. `UART_PINSETTER_COMMAND` from the Pi passes through any `CommandCode` the same way (generalized 2026-07-18 from a cycle-only message, so the Pi's REST API can also issue re-rack, not just cycle). ESP-NOW send is gated on the pinsetter being registered; RS485 send is gated only on the hardware being present (`RS485_ENABLED`) — so RS485 keeps working even if ESP-NOW registration never completed. |
...
| **Foul cooldown** | Per-lane, `FOUL_COOLDOWN_MS = 750`. A trip over the foul line produces several `LANE_FOUL` edges in quick succession (stumble, get up, re-break) — only the first within the 750ms window triggers `CMD_RERACK`; repeats are logged and suppressed. Bench-console `cycle`/`rerack` bypass this. |
