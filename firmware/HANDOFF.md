...
- **Gateway node** — learns downstream nodes via dynamic registration (no hardcoded MACs). The **pinsetter node**, not the gateway, owns the side → relay-channel mapping (see `PROTOCOL.md`). It also enforces a Pi-owned peer allowlist, so it no longer accepts any node that registers — see "Registration" below. On FOUL: just forwards the raw `MSG_LANE_EVENT` to the Pi, same as CLEAR — it no longer decides to RERACK itself (that used to happen here, off a local per-lane cooldown timer, independent of real game state; removed 2026-08-07, see `state_machine/state_machine.py`'s `on_foul()`/`FOUL_COOLDOWN_S`, which now owns both the debounce and the rerack decision). (There is no gateway→scoring command anymore — see `PROTOCOL.md`'s note on `CMD_FOUL`/`ScoringCommand` being removed, not just superseded.) Also the shared endpoint every node's RS485 fallback traffic terminates at.
- **Speed node** — covers *one lane pair*, two break-beams per side (4 sensors total). Does NOT time anything itself — emits a raw `MSG_BEAM_EVENT` (BROKEN/CLEAR, tagged upstream/downstream) per sensor edge, dual-sent on ESP-NOW and RS485, same as the fouling node emits per-side FOUL/CLEAR. The gateway forwards these to the Pi as-is; pairing the two timestamps into an interval and computing speed is the Pi bridge's job (event timing/scheduling lives off-node — see "Design principle" above and the UART bridge section below).
- **Ball detection node** — covers *one lane pair*, one break-beam per side, sited just **before the pin deck**. Announces "the ball arrived at the pins on side A/B" — the edge that kicks off vision capture → scoring → pinsetter cycle on the Pi. Emits the same `MSG_BEAM_EVENT` the speed node does, tagged `data[0] = ROLE_BALL_DETECT` (2) rather than `ROLE_DOWNSTREAM` (1); see `PROTOCOL.md` for why that distinction is load-bearing (a ball-detect edge wearing role 1 would get eaten by the Pi's speed pairing and produce a fabricated mph). Does no counting, timing, or correlation of its own — same pure-emitter shape as the fouling and speed nodes. Independent of the speed node in both directions: either can run alone on a side, and a side with both just produces a duplicate trigger that vision's in-flight guard absorbs.
...
Every downstream node sends a `NodeMessage` (`MSG_REGISTER`) **over ESP-NOW only** on startup **and re-sends it every 10s** (fouling, speed, ball detect, and pinsetter) so a rebooted gateway re-learns them without node power-cycles. The gateway calls `esp_now_add_peer` on the fly, records the node in a single shared `peers[]` table keyed by MAC (this replaced four type-split arrays — an allowlist is inherently one MAC→nodeType mapping), and **silently ignores re-registrations from nodes it already knows** — you should see exactly one `Registered ... node` line per node per gateway boot. Only the gateway's MAC (`68:25:DD:32:64:7C`) is hardcoded on the downstream nodes. RS485 never carries registration, for any node — it's a hardwired bus, nothing to discover; see "RS485 fallback bus (all nodes)" above.

**Which nodes are allowed to register is decided by the Pi, not the gateway.**
A node's hardcoded `GATEWAY_MAC` means a correctly-flashed one can only reach
its own gateway — but a misflashed one reaches whichever gateway that MAC
names, and since every leaf sketch is now byte-identical across pairs (they
report `laneSide`, not a lane number) its events are indistinguishable from the
real node's. So `uart_bridge` owns an allowlist of `{MAC, nodeType}`, pushes it
down as `UART_PEER_TABLE`, and the gateway caches it in NVS and enforces it on
every inbound ESP-NOW frame. Every registration attempt — **accepted or
refused** — comes back up as `UART_NODE_SEEN`, so a node being turned away is
visible instead of presenting as a mysteriously dead lane.

A gateway with no allowlist yet is **UNGOVERNED**: it accepts anyone, exactly
as before, so a fresh install still comes up unattended. Enforcement begins the
first time a table is pushed. Allowing a MAC is a deliberate operator action
(`POST /peers/{mac}/allow`) because nothing in software can tell "my fouling
node" apart from "the neighbouring pair's fouling node flashed with my
gateway's MAC". Enforcement is ESP-NOW-only — RS485 frames carry no sender
address, and that bus is trusted because **each pair has its own physically
isolated segment**, which is a requirement rather than a convention. See
`PROTOCOL.md`'s "Peer registry".
...
12. **Bench test everything this branch changed (2026-08-13, implemented, none of it flashed).** Five layers landed together and not one has touched hardware: the A/B lane-side rename, the shared `lanelink` library, the dual-send `DualSendFilter` on the gateway *and* the pinsetter, `Rs485Link::observeBus()` carrier sense on the three emitter nodes, and the NVS-backed peer allowlist. Everything below it in this list is now also downstream of those. Confirm in this order, because each step's failure mode masks the next: (a) each node registers exactly once and the gateway prints its allowlist state at boot; (b) `MSG_STATUS`/`MSG_LANE_EVENT` arrive at the Pi **once**, not twice, with both transports up — that is `DualSendFilter` working, and the 1500 ms window is an estimate from queue arithmetic (8 frames x 75 bytes @ 9600 baud), so measure the real ESP-NOW-vs-RS485 arrival gap and widen it if the copies straddle it; (c) an allowlist survives a gateway power cycle (real NVS round-trip, only ever exercised as a compile); (d) with a scope or a second listener, that the emitter nodes now actually hold off while another node transmits.
13. **Node identity in NVS + the pair-button provisioning handshake (2026-08-13, designed, not built).** The end state in `docs/index.md`'s peer-registration flows: a node stores `{provisioned, role, gatewayMac}` in NVS, rejoins silently on every boot, and only ever asks to be provisioned when a human presses a **pair button** on the board (user's call, 2026-08-13). That button is what makes the whole thing safe, and it is worth understanding why before building it:
    - **It eliminates the house-wide-reboot race** that the design-principles section at the top of this file warns about. That race only existed because discovery was imagined as automatic-on-boot; if an unprovisioned node sits idle until pressed, a whole house rebooting at once is a non-event. Provisioned nodes rejoin from NVS without beaconing at all.
    - **It disambiguates `/peers/pending`.** Today the Pi cannot tell one of its own nodes from a neighbouring pair's misflashed one (see `uart_bridge/peer_registry.py`), which is why allowing a MAC is a human decision. A node in pairing mode is by definition the one whose button was just pressed.
    - **Long-press to clear NVS restores the escape hatch.** Right now every failure on this branch is fixable by reflashing a known-good `GATEWAY_MAC`; moving identity into NVS gives that up unless there is a factory-reset gesture. Build the reset in the same pass as the provisioning, not after — a node that cannot be un-provisioned without a reflash is worse than what we have now.

    Two things still open:
    - **A pairing node has no `GATEWAY_MAC` yet, so it must broadcast, and every gateway in radio range hears it.** Recommend arming both ends: the button on the node *and* a provisioning window opened from the UI on the Pi, so accidental cross-pairing needs two simultaneous human actions on different lanes. The gateway should only forward pairing beacons upstream while its own window is open.
    - **GPIO0 is the free option and a trap.** Most ESP32 devkits already have a button on it, so no extra hardware — but it is a strapping pin, so holding it *at boot* enters the bootloader instead. That rules out "hold during power-on to factory reset"; a post-boot long-press still works. Also needs some pairing-mode feedback (the fouling node has a buzzer; the others have nothing but the onboard LED) — pressing a button with no indication of whether it took is its own failure mode.
14. **Gateway replacement / failover (designed in `docs/index.md`, not built, and blocked on item 13).** The compute node hands the dead gateway's MAC to its replacement over UART as a credential; the replacement broadcasts a claim; nodes re-pair only if the claimed MAC matches the one they already trusted. That verification needs the node-side stored `gatewayMac` from item 13, so this cannot land first. Two things to carry into the design: the credential is a bearer token broadcast in the clear (fine against the accidental threat model — a mis-set spare — but not against an adversary; ESP-NOW PMK/LMK encryption is the lever if that changes), and **the Pi must be the arbiter of whether the incumbent is actually dead**, since a degraded-but-alive gateway would otherwise leave two boards claiming one identity and nodes flapping between them. The Pi can decide because it holds the physical UART link, which is the only unforgeable "this one is mine" signal in the system.
